                                            ...
                                            <tbody>
                                                ...
                                                <tr>
                                                    ...
                                                    <td class="text-center p-2">
                                                        <a href="javascript:;" class="btn-sm btn btn-outline-info m-0" style="border:none;text-decoration: none; color:#0dcaf0; padding:5px 5px 2px 5px" data-id="<?= $prioritas['id']; ?>" data-prioritas="<?= $prioritas['prioritas']; ?>" data-bs-toggle="modal" title="edit" data-bs-target="#ModalEdit">
                                                            <span class=" mdi mdi-18px mdi-tooltip-edit" title="edit"></span>
                                                        </a>
                                                        <a href="/admin/prioritas/delete/{{ $prioritas->id }}" class="btn-sm btn btn-outline-danger m-0 delete-confirm" style="border:none;text-decoration: none; color:#dc3545; padding:5px 5px 2px 5px"><span class=" mdi mdi-18px mdi-delete-forever btn-delete" title="delete"></span></a>
                                                    </td>
                                                </tr>
                                                @endforeach
                                            </tbody>
...
<script>
    $(document).ready(function() {
        $('#ModalAdd').on('show.bs.modal', function(event) {
            ...
            var modal = $(this)
            $('.modal-body form').attr('action', '/admin/prioritas/add');
        });
        $('#ModalEdit').on('show.bs.modal', function(event) {
            var div = $(event.relatedTarget) // Tombol dimana modal di tampilkan
            var modal = $(this)
            $('.modal-body form').attr('action', '/admin/prioritas/update/' + div.data('id'));
            modal.find('#nama').attr("value", div.data('prioritas'));
            modal.find('#id_prioritas').attr("value", div.data('id'));
        });
        $('#example').DataTable();
    });
</script>
        ...
        <div class=" modal-content">
            <div class="modal-header p-0">
                <div class="d-flex justify-content-between align-items-center mb-3 p-2" style="margin:0px !important;">
                    <h4 style="text-align: center; padding:10px 10px;margin:0px !important">Tambah prioritas</h4>
                </div>
                <button type="button" class="btn-close" style="padding-left:40px" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
                <form action="/admin/prioritas" method="post">
                    @csrf
                    <div class="row">
                        <div class="col-md-12">
...
<div class="modal fade" id="ModalEdit" tabindex="-1" aria-labelledby="exampleModalLabe2" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered" style="margin: auto; width: 50%; padding: 10px;">
        <div class="modal-content">
            <div class="modal-header p-0">
                <div class="d-flex justify-content-between align-items-center mb-3 p-2" style="margin:0px !important;">
                    <h4 style="text-align: center; padding:10px 10px;margin:0px !important">Edit prioritas</h4>
                </div>
                <button type="button" class="btn-close" style="padding-left:40px" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
